...
func InitRedis() *redis.Pool {
	...
	RedisConn = &redis.Pool{
		...
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", fmt.Sprintf("%s:%s", reidsConf.Address, reidsConf.Port))
			if err != nil {
				return nil, err
			}
			// 验证密码 这里本地环境没密码,不验证
			//_, err = c.Do("auth", reidsConf.Password)
			// 选择 db
			_, err = c.Do("select", reidsConf.Db)
			if err != nil {
